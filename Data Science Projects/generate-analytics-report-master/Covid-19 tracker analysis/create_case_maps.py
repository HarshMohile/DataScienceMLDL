...
def plot_india_case_map(filename=None, day=None):
	df = load_relevant_data()
	dates = list(df.columns)
	df = df.groupby('Province/State')[dates].agg('sum')
	create_India_figure(df, filename, day)
...
def create_India_figure(df, filename, day):
	...
	fig = px.choropleth(df,
                    ...
                    title=f"India Daily Cases, {day}",
                    width=1000,
                    #height=500,
                    range_color=[0,3000])

	fig.update_layout(margin=dict(l=0, r=0, t=70, b=0), title={"font": {"size": 20}, "x":0.5},)
	filename = filename if filename else "india_chart.png"
...
def create_global_figure(df, filename, day):
	...
	fig = px.choropleth(df,
                    ...
                    scope="India", # Try 'europe', 'africa', 'asia', 'south america', 'north america'
                    color="Cases",
                    hover_name="Country",
                    #projection="miller",
                    color_continuous_scale='Peach',
...
if __name__ == '__main__':
	...
	plot_india_case_map(day=yesterday) # saves as usa_chart.png by default
